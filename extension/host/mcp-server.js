...
function getPort() {
  const configPath = path.join(os.homedir(), ".config", "paladin-qa", "config.json");
  try {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (mode === "primary") {
      if (!nativeHostSocket || nativeHostSocket.destroyed) {
        clearTimeout(timer);
        pendingRequests.delete(id);
        reject(new Error("Browser extension is not connected. Make sure a supported Chromium browser is running with the Paladin QA extension installed and enabled."));
        return;
      }
...
const pidfilePath = path.join(os.tmpdir(), `paladin-qa-mcp-${TCP_PORT}.pid`);
...
function startClientMode() {
  ...
  function connect() {
    ...
    primarySocket.on("close", () => {
      ...
      pendingRequests.clear();
      // Try to become primary ourselves (port may be free now).
      // Fall back to reconnecting as client if another session grabbed it first.
      setTimeout(() => {
        ...
        tcpServer.once("error", (err) => {
          if (err.code === "EADDRINUSE") {
            process.stderr.write(`Port still taken, reconnecting as client...\n`);
            connect();
          } else {
            process.stderr.write(`TCP server error on promotion: ${err.message}\n`);
          }
        });
      }, 500);
    });
...
// Find first image part in an MCP result and persist it to disk as a binary file.
// Returns the absolute resolved path written. Throws if no image part exists.
function saveImagePartToDisk(result, filePath) {
  const parts = (result && result.content) || [];
  const imagePart = parts.find(p => p && p.type === "image" && typeof p.data === "string");
  if (!imagePart) throw new Error("no image part in tool result");
  const abs = path.isAbsolute(filePath) ? filePath : path.resolve(process.cwd(), filePath);
  fs.mkdirSync(path.dirname(abs), { recursive: true });
  fs.writeFileSync(abs, Buffer.from(imagePart.data, "base64"));
  return abs;
}
...
server.tool(
  ...
  "Use a mouse and keyboard to interact with a web browser, and take screenshots. If you don't have a valid tab ID, use tabs_context_mcp first to get available tabs.\n* Whenever you intend to click on an element like an icon, you should consult a screenshot to determine the coordinates of the element before moving the cursor.\n* If you tried clicking on a program or link but it failed to load, even after waiting, try adjusting your click location so that the tip of the cursor visually falls on the element that you want to click.\n* Make sure to click any buttons, links, icons, etc with the cursor tip in the center of the element. Don't click boxes on their edges unless asked.",
  {
    ...
    save_to_disk: z.string().optional().describe("Absolute (or cwd-relative) file path to persist the captured PNG to disk. Only applies to `screenshot` and `zoom` actions; ignored otherwise. Parent directories are created if missing. On success a text part is appended with the resolved path."),
  },
  async (args) => {
    const { save_to_disk, ...rest } = args;
    const result = await callTool("computer", rest);
    if (save_to_disk && (args.action === "screenshot" || args.action === "zoom")) {
      try {
        const abs = saveImagePartToDisk(result, save_to_disk);
        (result.content ||= []).push({ type: "text", text: `Screenshot saved to ${abs}` });
      } catch (e) {
        (result.content ||= []).push({ type: "text", text: `save_to_disk failed: ${e.message}` });
      }
    }
    return result;
  }
