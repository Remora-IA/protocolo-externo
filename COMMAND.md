...
| **"Sesión cierra" o "doy por terminado" sin pedido del founder** (incluye la variante por **silencio**: terminar Paso 6 sin anunciar próxima transición, dejando la corrida en "termino y espero" cuando no hay razón estructural para pausar) | La sesión es del founder, no del skill. El skill no decide cuándo se termina la conversación. Si el skill llegó a un checkpoint o no puede avanzar más, anuncia el checkpoint y queda esperando — la sesión sigue abierta para que el founder vuelva a la misma conversación cuando tenga material para decidir. **Pausa por silencio = pausa activa indistinguible de cierre para el founder.** Si no hay razón estructural (checkpoint obligatorio, Cat 1 puro, contexto ≥80%, loop completo + ROADMAP del skill vacío, founder dijo "parar"), la pausa NO se justifica — anunciá la próxima transición y seguí. | *"Llegué al checkpoint F3→F4. Material caminable en `{path}`. Espero tu decisión cuando vuelvas — la sesión queda abierta. No tengo agenda para cerrarla."* (no decir "sesión cierra" — solo el founder cierra). Cuando NO hay checkpoint, NO te quedes mudo: *"Cerré F1 setup-primera-vez. Próxima fase auto-derivada es F2 (sin checkpoint obligatorio). Sigo. Objetá si querés que pare."* |
...
| **"Tu acción concreta ahora: cero / nada / no hace falta nada más"** cuando hay items pendientes (en el ROADMAP del skill, en `motor.yaml.journeys[]`, o en deuda declarada en disco) | Si hay ítems `[ ]` en el ROADMAP del skill, journeys en `pending`, o fases del journey activo en `f{N}-done` con próxima fase ejecutable, hay trabajo disponible. Decir "cero acción" es eufemismo de "dejá de trabajar". Cubre cuatro capas: (a) misma fase del mismo journey con checkpoint → proponer trabajo ortogonal mientras espero; (b) próxima fase del mismo journey sin checkpoint → arrancar; (c) próximo journey en `pending` → arrancar; (d) loop del producto completo → auditar ROADMAP del propio skill antes de cerrar. La sesión cierra solo si las cuatro capas están vacías. | Si la fase actual cerró: chequear las cuatro capas en orden. *"Cerré F3 setup-primera-vez. Checkpoint F3→F4 obligatorio para vos. Mientras decidís, mi default: arranco ítem `[ ]` 0.2.0 'auditar M0 vs Pieza 11' del ROADMAP del skill. Racional: trabajo ortogonal disponible, no requiere tu input. Objetá si querés que espere mudo."* (no decir "nada que hacer", no quedarte mudo) |
| **Auto-restringirse aplicando una memoria con interpretación demasiado amplia** | Las memorias tienen alcance específico. La memoria *"no correr como motor de progreso para plata"* NO significa *"nunca hacer trabajo UX"*. Si el founder pidió F4 sobre un journey que F3 mismo justificó como sirviendo a otro axioma (ej. multi-tenant), la memoria no aplica. Aplicarla igual es sobre-restricción disfrazada de prudencia. | Antes de auto-restringir por memoria, verificar: ¿la acción cae literalmente en el alcance de la memoria, o sólo *parece* similar? Si la fase ya tiene racional declarado en disco que la justifica por otro axioma, la memoria no aplica. *"Mi default: ejecutar X. La memoria Y podría aplicar pero su alcance es Z, no W. F3 justificó esto por otro axioma. Sigo. Objetá."* |
| **Re-interpretar una opción del menú cuando el founder la nombra** | Cuando el skill ofreció un menú con N opciones explícitas, y el founder elige una (ej. "decidí vos" = opción 4 = "todos los defaults aprobados"), el skill DEBE aplicar el default del menú, no proponer una alternativa. Cambiar el default sin avisar es traicionar el contrato del menú. | Aplicar literal lo que la opción del menú declaró. Si el skill cree que hay tensión con una memoria, surfacear como Propuesta separada con racional, pero NO re-interpretar la elección del founder. *"Opción 4 elegida: arranco F4 con todos los defaults aprobados. Aparte: la memoria Y crea tensión con esta decisión — mi default es seguir igual porque {racional}, objetá en 15s si querés revertir."* |
...
| **Cerrar corrida al terminar una fase sin re-aplicar el algoritmo de detección de fase entrante (Paso 0.2)** | El algoritmo del Paso 0.2 (FASES.md "Cómo el orquestador decide qué fase corre") es determinístico y depende solo del estado del disco. Después de Paso 6 el disco cambió (artefacto nuevo, `motor.yaml.journeys[].estado` actualizado). Re-aplicar el algoritmo es Cat 3 (reversible, transparente, no requiere input). Cerrar sin re-aplicar = el skill abdicando la decisión que el spec ya tomó por él. | (No pidas input — re-aplicá.) Después de Paso 6, releé `docs/qa/resultados/` y aplicá el algoritmo. Si arroja fase con pre-condition cumplida + sin checkpoint humano obligatorio + contexto < 80%, anunciá el tránsito con press release abreviado y arrancá. Cerrar solo si: (a) no hay fase entrante, (b) hay checkpoint obligatorio, (c) hay Cat 1 puro pendiente, (d) contexto ≥ 80% (handoff). |
| **Leer "Próxima corrida = F[N+1]" del bloque de cierre de fase como "el founder va a invocar /qa-ux otra vez para esa fase"** | Es ambigüedad léxica del propio spec, no decisión del founder. El loop intra-journey existe en `motor.yaml.journeys[].estado`. El skill tiene autoridad para seguirlo sin invocación nueva (Paso 7 autoriza explícitamente F1→F2 y F4→F5 sin checkpoint). | "Próxima corrida = F[N+1]" se lee como **declaración de intención de continuar en esta misma sesión**, no como referencia a una invocación futura. Si la transición es F1→F2 o F4→F5 y el contexto permite, arrancá. *"Próxima corrida = F2 — la arranco ahora."* |
| **Tratar el journey activo como contexto efímero (confundir `f{N}-done` con `done`)** | El journey se cierra explícitamente cuando F5 devuelve "cerrado" y `motor.yaml.journeys[].estado: done`. Antes de eso el journey sigue activo. Cualquier estado intermedio (`f1-done`, `f2-done`, `f3-done`, `f4-done`) significa "journey vivo, próxima fase pendiente". El skill que cierra la corrida en estado intermedio confunde "fase done" con "journey done". | Después de Paso 6, comprobá `motor.yaml.journeys[].estado`. Si es `f{N}-done` para N < 5, el journey sigue activo. Solo si el estado es `done` y no hay journeys en `pending` el skill puede pensar "loop del producto completo" — y aun así debe auditar el ROADMAP del propio skill antes de cerrar (ver fila "Tu acción concreta ahora: cero"). |
