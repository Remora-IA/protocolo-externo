...
| **"Tu acción concreta ahora: cero / nada / no hace falta nada más"** cuando hay items pendientes en el ROADMAP | Si el ROADMAP tiene ítems `[ ]` y el founder está activo, hay trabajo disponible. Decir "cero acción" es eufemismo de "dejá de trabajar". Es el mismo modo de falla que el anterior, en otra capa: la sesión termina su iteración y propone "nada" en vez de proponer el próximo ítem. | Si la fase actual cerró: leer ROADMAP, proponer el primer ítem `[ ]` de la versión activa con racional. *"Cerré F4. Próximo ítem del 0.2.0: {ítem}. Mi default: arranco {acción concreta}. Objetá si querés otro orden."* (no decir "nada que hacer") |
